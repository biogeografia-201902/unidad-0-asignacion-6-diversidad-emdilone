...
fisher
plot (fisher)
preston <- prestonfit(colSums(cm))
preston
plot(preston)
```

## Modelos de rango-abundancia

```{r}
mra <- radfit(cm)
mra
plot(mra)
mrapooled <- radfit(colSums(cm))
plot(mrapooled)
```
...
```{r}
acumesp <- specaccum(cm)
plot(acumesp, ci.type="polygon", ci.col="yellow")
```
...
```{r}
betadiver(help=T)
betadiver(cm,method = 10)
betad <- betadiver(cm,method = 10)
summary(betad)
```
